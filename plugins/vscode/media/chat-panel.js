(function () {
	...
	const slashCommandChip = document.getElementById("slash-command-chip");
	...
	const SLASH_COMMANDS = [
		{
			id: "test",
			name: "/test",
			description: "Generate tests for pasted code",
		},
		{
			id: "explain",
			name: "/explain",
			description: "Explain code, errors, or stack traces",
		},
		{
			id: "doc",
			name: "/doc",
	...
	function initDropdowns() {
		class CustomDropdown {
			...
			setOptions(options, selectedValue) {
				...
				options.forEach((opt) => {
					...
					item.className =
						"px-3 py-1.5 cursor-pointer hover:bg-vscode-list-hover text-[0.9em] transition-colors";
					if (opt === selectedValue) {
						item.classList.add(
							"bg-vscode-list-active",
							"text-vscode-list-activeFg",
						);
					} else {
		...
		modeDropdown = new CustomDropdown(
			"mode-trigger",
			"mode-dropdown",
			"mode-trigger-label",
			(val) => {
	...
	function createMessageFooter(getText, role, sentAt) {
		...
		footer.className =
			"flex h-5 items-center gap-1.5 mt-1 text-xs text-vscode-fg opacity-60 " +
			(role === "user" ? "self-end" : "self-start");

		const btn = document.createElement("button");
		btn.type = "button";
		btn.className =
			"flex items-center justify-center bg-transparent border-none cursor-pointer text-vscode-fg opacity-60 hover:opacity-100 p-1 rounded hover:bg-vscode-toolbarHover [&_svg]:mr-0 mb-1";
		btn.title = "Copy";
		...
		timeEl.textContent = sentAt.toLocaleTimeString([], {
			hour: "numeric",
			minute: "2-digit",
		});
	...
	function setSelectedSlashCommand(command) {
		...
		slashCommandChip.innerHTML = "";

		if (!command) {
			slashCommandChip.classList.add("hidden");
			return;
		}

		const chip = document.createElement("span");
		chip.className =
			"inline-flex items-center gap-1 rounded-full bg-vscode-toolbarHover px-2 py-0.5";
		chip.textContent = command.name;

		const removeBtn = document.createElement("button");
		removeBtn.type = "button";
		removeBtn.className =
			"bg-transparent border-none text-vscode-fg opacity-80 cursor-pointer leading-none p-0";
		removeBtn.title = "Remove quick action";
		...
		slashCommandChip.appendChild(chip);
		slashCommandChip.classList.remove("hidden");
	}

	function applySlashSelection(command) {
		const currentValue = chatInput.value;
		const beforeCursor = currentValue.slice(0, chatInput.selectionStart);
		const trailingMatch = beforeCursor.match(/(?:^|\s)\/[a-z-]*$/i);

		if (trailingMatch) {
			const replaceFrom = beforeCursor.length - trailingMatch[0].length;
			const cleaned =
				currentValue.slice(0, replaceFrom) +
				currentValue.slice(chatInput.selectionStart);
	...
	function renderSlashDropdown(commands) {
		...
		commands.forEach((command, index) => {
			...
			item.type = "button";
			item.className =
				"w-full text-left bg-transparent border-none px-3 py-2 cursor-pointer transition-colors flex items-start justify-between gap-3";
			...
			left.className = "font-semibold text-[0.88em]";
			left.textContent = command.name;

			const right = document.createElement("span");
			right.className = "text-[0.8em] opacity-75";
	...
	function updateSlashAutocomplete() {
		const cursorAtEnd = chatInput.selectionStart === chatInput.value.length;
		if (!cursorAtEnd) {
			hideSlashDropdown();
			return;
		}
	...
	chatInput.addEventListener("keydown", (e) => {
		if (
			!slashDropdown.classList.contains("hidden") &&
			...
			if (e.key === "ArrowUp") {
				e.preventDefault();
				slashSelectedIndex =
					(slashSelectedIndex - 1 + slashSuggestions.length) %
	...
	function submitMessage() {
		...
		vscode.postMessage({
			type: "submitMessage",
			text: text,
			command: selectedSlashCommand ? selectedSlashCommand.id : undefined,
		});
	...
	function appendMessage(content, role) {
		...
		wrapper.className =
			"group flex flex-col min-w-0 shrink-0 " +
			(role === "user"
				? "self-end items-end max-w-[85%]"
				: "self-start items-start max-w-full");

		const msgEl = document.createElement("div");
		msgEl.className =
			"leading-snug break-words min-w-0 w-full " +
			(role === "user"
	...
	function appendChunk(textChunk) {
		...
		if (!currentTurnEl || !currentTextEl) {
			...
			const wrapper = document.createElement("div");
			wrapper.className =
				"group flex flex-col min-w-0 self-start items-start max-w-full";
	...
	function renderSessions() {
		...
		sessionsData
			...
			.forEach((session) => {
				const label =
					session.title || session.cwd || session.sessionId.slice(0, 8);
		...
		Object.entries(groups).forEach(([groupName, sessions]) => {
			...
			if (groupName !== "Older") {
				...
				groupHeader.className =
					"px-4 py-1.5 text-[0.78em] font-semibold uppercase tracking-[0.06em] opacity-50";
			...
			sessions.forEach((session) => {
				...
				itemEl.className =
					"flex items-center px-4 py-1.5 cursor-pointer gap-2 rounded mx-1 transition-colors hover:bg-vscode-list-hover group";

				const labelEl = document.createElement("span");
				labelEl.className =
					"flex-1 overflow-hidden text-ellipsis whitespace-nowrap text-[0.9em]";
				...
				deleteBtn.className =
					"bg-transparent border-none cursor-pointer text-vscode-fg opacity-0 group-hover:opacity-100 transition-opacity p-1 flex items-center justify-center hover:bg-vscode-toolbarHover rounded";
	...
	function handleSyncState(message) {
		...
		if (modeDropdown)
			modeDropdown.setOptions(message.availableModes, message.mode);
		if (modelDropdown)
			modelDropdown.setOptions(message.availableModels, message.model);
	}
	...
	function handleAcpUpdate(payload) {
		...
		if (update.sessionUpdate === "user_message_chunk") {
			...
			update.sessionUpdate === "prompt_response" ||
	...
	class ThoughtAggregator {
		constructor() {
			...
			this.header.className =
				"flex items-center gap-1.5 cursor-pointer opacity-70 text-vscode-fg hover:opacity-100 transition-opacity select-none w-fit";
			...
			this.body.className =
				"mt-2 pl-3 border-l-[3px] border-vscode-border opacity-70 text-vscode-fg markdown-body text-[0.95em]";
	...
	class ToolAggregator {
		constructor() {
			this.el = document.createElement("div");
			this.el.className =
				"my-3 border border-vscode-widget-border rounded bg-vscode-widget-bg overflow-hidden shrink-0 tool-aggregator";

			this.header = document.createElement("div");
			this.header.className =
				"px-3 py-2 flex items-center bg-vscode-widget-header border-b border-vscode-widget-border gap-2 cursor-pointer select-none";
			this.header.onclick = () => this.toggle();
		...
		addOrUpdateTool(toolCallId, update) {
			...
			if (!item) {
				...
				item.className =
					"px-3 py-1.5 border-t border-vscode-widget-border flex flex-col gap-2 text-[0.85em] font-vscode first:border-t-0";
				...
				status.className =
					"tool-status flex items-center justify-center mt-0.5 shrink-0";
			...
			if (statusEl) {
				if (update.status === "success" || update.status === "completed") {
					...
					(update.status === "failed" &&
						...
						(update.rawOutput.includes("AbortError") ||
							update.rawOutput.includes("cancelled")))
				) {
					statusEl.innerHTML = ICONS.cancelled;
	...
	function handlePlanUpdate(update) {
		...
		if (!card) {
			...
			card.id = "plan-card";
			card.className =
				"my-3 border border-vscode-widget-border rounded bg-vscode-widget-bg overflow-hidden shrink-0";

			const header = document.createElement("div");
			header.className =
				"px-3 py-2 flex items-center bg-vscode-widget-header border-b border-vscode-widget-border gap-2";
		...
		for (const entry of entries) {
			...
			row.className =
				"px-3 py-1.5 border-t border-vscode-widget-border first:border-t-0 flex items-start gap-2 text-[0.85em] font-vscode";
	...
	function handleToolCallUpdate(update) {
		const toolCallId =
			update.toolCallId || (update.toolCall && update.toolCall.toolCallId);
		...
		const toolName =
			update.name || (update.toolCall && update.toolCall.name) || "";
		const isMutating = [
	...
	function createEditCard(toolCallId, update) {
		...
		card.className =
			"my-2 flex items-center justify-between px-3 py-2 border border-vscode-widget-border rounded bg-vscode-editor-bg cursor-pointer hover:bg-vscode-list-hover group tool-card";
		...
		hoverBtn.className =
			"opacity-0 group-hover:opacity-100 transition-opacity bg-vscode-button-secondary text-vscode-fg px-2 py-0.5 rounded text-[0.85em]";
	...
	function handlePermissionRequested(toolCallId, toolCall, options) {
		...
		if (options && Array.isArray(options) && options.length > 0) {
			...
			actionsDiv.className =
				"tool-actions px-3 py-3 bg-vscode-widget-header border-t border-vscode-widget-border flex flex-col gap-2";
			...
			for (const opt of options) {
				const btn = document.createElement("button");
				btn.className =
					"w-full text-left bg-transparent border border-vscode-button-secondary text-vscode-fg hover:bg-vscode-button-secondaryHover rounded px-3 py-1.5 cursor-pointer font-vscode text-[0.9em] transition-colors";
			...
			actionsDiv.className =
				"px-3 py-2 bg-vscode-widget-header border-t border-vscode-widget-border flex justify-end gap-2 tool-actions";

			const approveBtn = document.createElement("button");
			approveBtn.className =
				"border-none rounded px-3 py-1.5 cursor-pointer font-vscode text-[0.9em] transition-colors bg-vscode-button-bg text-vscode-button-fg hover:bg-vscode-button-hover";
			...
			denyBtn.className =
				"bg-transparent border border-vscode-button-secondary text-vscode-fg hover:bg-vscode-button-secondaryHover rounded px-3 py-1.5 cursor-pointer font-vscode text-[0.9em] transition-colors";

...
| `/review` | Review a branch or PR diff for bugs, security issues, and style violations. Usage: `/review <branch-or-pr-number>` (e.g. `/review main`, `/review 42`). Add `quick` for the fast one-shot pass or `deep` for three specialist finders with deduplication — see [Agentic Review](review.md) |

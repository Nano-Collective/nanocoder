...
test('escapeMarkdown escapes formatting characters', t => {
    if (!escapeMarkdown) {
        t.pass('Skipping test - escapeMarkdown not exported or available');
        return;
    }

    const unescaped = '[link](https://evil.com) *bold* _italic_ `code` <tag> #head';
    const escaped = escapeMarkdown(unescaped);

    t.is(escaped, '\\[link\\]\\(https://evil\\.com\\) \\*bold\\* \\_italic\\_ \\`code\\` \\<tag\\> \\#head');
});

test('sanitizeUrl strips angle brackets and whitespace', t => {
    if (!sanitizeUrl) {
    ...
    t.is(cleanUrl, 'https://example.com/foobarbaz');
});

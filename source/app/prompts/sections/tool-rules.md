...
- When calling tools that modify files or execute commands (`execute_bash`, `string_replace`, `write_file`), provide a concise `description` parameter summarizing the intent or rationale of the action so the user understands why it is needed.

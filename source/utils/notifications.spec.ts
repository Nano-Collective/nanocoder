...
// ============================================================================
// Linux & Windows & Daemon Notification Tests
// ============================================================================
...
test.serial('sendNotification on win32 invokes powershell', (t) => {
	const originalPlatform = process.platform;
	const originalExecFile = childProcess.execFile;
	let executedCommand = '';
	let executedArgs: string[] = [];
	...
	try {
		setNotificationsConfig({
			enabled: true,
			events: {questionPrompt: true},
		});

		t.notThrows(() => sendNotification('questionPrompt'));
		t.is(executedCommand, 'powershell');
		t.is(executedArgs[0], '-NoProfile');
		t.is(executedArgs[1], '-Command');
		t.true(executedArgs[2].includes('System.Windows.Forms.NotifyIcon'));
	} finally {
		childProcess.execFile = originalExecFile;
		Object.defineProperty(process, 'platform', {
			value: originalPlatform,
